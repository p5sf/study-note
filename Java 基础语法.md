...
抽象类
> [!attention]- 注意不能和那些关键字共存
> - private:私有内容子类继承不到，所以，不能重写
> 

使用：不能和哪些关键词共存？①。但是abstract修饰的方法，要求被重写。两者冲突。②final：final修饰的方法不能被重写。③static:假如一个抽象方法能通过static修饰，那么这个方法，就可以直接通过类名调用。而抽象方法是没有方法体的，这样的调用无意义。所以，不能用static修饰。
